...
const ModernUniversityProfile: React.FC = () => {
  ...
  const university = useMemo(() => {
    ...
    return ALL_SOUTH_AFRICAN_UNIVERSITIES.find(uni => uni.id === universityId) || null;
  }, [universityId]);
  ...
  const stats = useMemo(() => {
    ...
    const programCount = university.faculties?.reduce((total, faculty) => {
      return total + (faculty.degrees?.length || 0);
    }, 0) || 0;

    const averageAPS = university.faculties?.reduce((total, faculty) => {
      const facultyAvg = faculty.degrees?.reduce((sum, degree) => sum + degree.apsRequirement, 0) || 0;
  ...
  const handleVisitWebsite = () => university?.website && window.open(university.website, '_blank');
  const handleViewBooks = () => navigate(`/books?university=${university?.id}`);
  const handleApply = () => university?.website && window.open(`${university.website}/admissions`, '_blank');
  ...
  const facultyInsights = useMemo(() => {
    ...
    return university.faculties.map(faculty => ({
      ...
      avgAPS: Math.round((faculty.degrees?.reduce((sum, d) => sum + d.apsRequirement, 0) || 0) / (faculty.degrees?.length || 1)),
      competitiveness: faculty.degrees?.some(d => d.apsRequirement > 30) ? 'High' : 'Moderate'
    }));
  }, [university]);
      ...
      <>
        ...
        <div className="min-h-screen bg-slate-50 flex items-center justify-center">
          <div className="text-center max-w-md mx-auto p-8">
            ...
            <p className="text-slate-600 mb-6">The university you're looking for doesn't exist or has been moved.</p>
            <Button onClick={handleBack} className="bg-slate-900 hover:bg-slate-800">
          ...
          <div className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div className="flex items-start gap-8">
              ...
              <div className="bg-white rounded-2xl p-4 w-24 h-24 flex items-center justify-center flex-shrink-0 shadow-xl">
                ...
                <Building className={`h-12 w-12 text-slate-400 ${university.logo ? 'hidden' : ''}`} />
              </div>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-slate-300 text-lg leading-relaxed max-w-3xl">
                      {university.overview || `Leading public research university offering world-class education and innovative research opportunities across multiple disciplines.`}
                    </p>
                  </div>
          ...
          <Tabs defaultValue="overview" className="w-full">
            ...
            <TabsList className="bg-white border border-slate-200 shadow-sm mb-8 h-auto p-1">
              {[
                ...
                <TabsTrigger
                  key={tab.value}
                  value: stats?.graduationRate ? `${stats.graduationRate}%` : "87%",
            ...
            <TabsContent value="overview" className="space-y-8">
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-slate-900">{stats?.graduationRate}%</span>
                      </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-slate-900">{stats?.employmentRate}%</span>
                      </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-slate-900">{stats?.acceptanceRate}%</span>
                      </div>
                    ...
                    <div className="space-y-4">
                      {facultyInsights.slice(0, 4).map((faculty, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                          <div>
                            <div className="font-medium text-slate-900 text-sm">{faculty.name}</div>
                            <div className="text-slate-500 text-xs">{faculty.programs} programs</div>
                          </div>
                          <Badge
                            variant={faculty.competitiveness === 'High' ? 'destructive' : 'secondary'}
                            className="text-xs"
                          >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <Card className="bg-white border-0 shadow-lg">
                  <CardHeader>
                    <CardTitle className="text-slate-900">About the University</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-slate-600 leading-relaxed mb-6">
                      {university.overview || `${university.fullName || university.name} stands as one of South Africa's premier institutions of higher learning, combining academic excellence with cutting-edge research and innovation. Our commitment to transformation and inclusivity makes us a leader in African higher education.`}
                    </p>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-slate-50 rounded-lg text-center">
                        <Users className="h-6 w-6 text-blue-500 mx-auto mb-2" />
                        <div className="text-2xl font-bold text-slate-900">{stats?.students.toLocaleString()}</div>
                        <div className="text-slate-600 text-sm">Total Students</div>
                      </div>
                      <div className="p-4 bg-slate-50 rounded-lg text-center">
                        <BookOpen className="h-6 w-6 text-emerald-500 mx-auto mb-2" />
                        <div className="text-2xl font-bold text-slate-900">{stats?.faculties}</div>
                    ...
                    <div className="space-y-4">
                      <div className="p-4 border border-amber-200 bg-amber-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-amber-800">Top University in Africa</div>
                            <div className="text-amber-600 text-sm">QS World University Rankings 2024</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-4 border border-blue-200 bg-blue-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-blue-800">Research Excellence</div>
                            <div className="text-blue-600 text-sm">Leading research output in South Africa</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-4 border border-emerald-200 bg-emerald-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-emerald-800">Transformation Leader</div>
                            <div className="text-emerald-600 text-sm">Excellence in diversity and inclusion</div>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <TabsContent value="programs" className="space-y-8">
              <div className="text-center mb-8">
                ...
                <p className="text-slate-600 text-lg max-w-2xl mx-auto">
                  Explore our comprehensive range of undergraduate and postgraduate programs across {university.faculties?.length} faculties.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-1">
                  <Card className="bg-white border-0 shadow-lg sticky top-6">
                    <CardHeader>
                      <CardTitle className="text-slate-900">Faculties</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2">
                      {university.faculties?.map((faculty, index) => (
                        ...
                        >
                          ...
                          <div className={`text-sm ${selectedFaculty === faculty.id ? 'text-slate-300' : 'text-slate-500'}`}>
                  ...
                  {selectedFaculty ? (
                      ...
                      return (
                        <Card className="bg-white border-0 shadow-lg">
                          <CardHeader className="border-b border-slate-100">
                            <CardTitle className="text-2xl text-slate-900">{faculty.name}</CardTitle>
                            <p className="text-slate-600">{faculty.description}</p>
                          </CardHeader>
                          <CardContent className="pt-6">
                            <div className="grid gap-4">
                              {faculty.degrees?.map((degree, index) => (
                                <div key={degree.id || index} className="p-4 border border-slate-200 rounded-lg hover:shadow-md transition-shadow">
                                  <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                      <h4 className="font-semibold text-slate-900 mb-2">{degree.name}</h4>
                                      <p className="text-slate-600 text-sm mb-3">{degree.description}</p>
                                      <div className="flex flex-wrap gap-2">
                                        ...
                                        >
                                          {degree.apsRequirement > 30 ? 'Highly Competitive' : 'Competitive'}
                                        </Badge>
                                      </div>
                                    </div>
                    ...
                    <Card className="bg-white border-0 shadow-lg">
                      <CardContent className="text-center py-12">
                        ...
                        <h3 className="text-xl font-semibold text-slate-900 mb-2">Select a Faculty</h3>
                        <p className="text-slate-600">Choose a faculty from the list to view available programs and details.</p>
                      </CardContent>
                    </Card>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <Card className="bg-white border-0 shadow-lg">
                  ...
                  <CardContent className="space-y-6">
                    <div className="space-y-4">
                      <div>
                        <label className="text-slate-500 text-sm font-medium mb-2 block">Website</label>
                        <a
                      ...
                      <div>
                        <label className="text-slate-500 text-sm font-medium mb-2 block">Address</label>
                ...
                <Card className="bg-white border-0 shadow-lg">
                  ...
                  <CardContent className="space-y-3">
                    <Button
                      onClick={() => window.open(`${university.website}/admissions`, '_blank')}
                      variant="outline"
                    ...
                    <Button
                      onClick={() => window.open(`${university.website}/academics`, '_blank')}
                      variant="outline"
                      className="w-full justify-start h-12"
                    >
                    ...
                    <Button
                      onClick={() => window.open(`${university.website}/student-life`, '_blank')}
                      variant="outline"
                      className="w-full justify-start h-12"
                    >
                    ...
                    <Button
                      onClick={() => window.open(`${university.website}/research`, '_blank')}
                      variant="outline"
                      className="w-full justify-start h-12"
                    >
            ...
            <TabsContent value="admissions">
              <Card className="bg-white border-0 shadow-lg">
                <CardContent className="text-center py-12">
                  ...
                  <h3 className="text-xl font-semibold text-slate-900 mb-2">Admissions Information</h3>
                  <p className="text-slate-600 mb-6">Detailed admissions requirements and application process coming soon.</p>
            ...
            <TabsContent value="research">
              <Card className="bg-white border-0 shadow-lg">
                <CardContent className="text-center py-12">
                  ...
                  <h3 className="text-xl font-semibold text-slate-900 mb-2">Research Excellence</h3>
                  <p className="text-slate-600">Research information and opportunities coming soon.</p>
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="campus">
              <Card className="bg-white border-0 shadow-lg">
                <CardContent className="text-center py-12">
                  ...
                  <h3 className="text-xl font-semibold text-slate-900 mb-2">Campus Life</h3>
                  <p className="text-slate-600">Campus facilities and student life information coming soon.</p>
                </CardContent>
              </Card>
            </TabsContent>
          </Tabs>
