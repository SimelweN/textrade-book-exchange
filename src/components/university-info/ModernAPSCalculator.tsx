...
const SAMPLE_SUBJECTS: APSSubject[] = [
  ...
  { name: "Afrikaans First Additional Language", marks: 68, level: 4, points: 5 },
];
...
const getPerformanceLevel = (aps: number): { level: string; color: string; description: string } => {
  if (aps >= 36) return {
    level: 'Outstanding',
...
const ModernAPSCalculator: React.FC = () => {
  ...
  const [activeInsight, setActiveInsight] = useState<'overview' | 'universities' | 'programs'>('overview');
  const [selectedFilter, setSelectedFilter] = useState<'all' | 'eligible' | 'competitive'>('all');
  const [showAllUniversities, setShowAllUniversities] = useState(false);

  // Calculate total APS
  const totalAPS = useMemo(() => {
    const total = subjects.reduce((total, subject) => total + (subject.points || 0), 0);
  ...
  const availableSubjects = useMemo(() => {
    return SOUTH_AFRICAN_SUBJECTS.filter(subject =>
      !subjects.some(s => s.name === subject) || subject.includes("Select")
    );
  }, [subjects]);

  // Comprehensive degree analysis - Show all programs with eligibility status
  const degreeAnalysis = useMemo(() => {
    const degrees: DegreeInsight[] = [];

    // Debug logging
    if (import.meta.env.DEV) {
      console.log(`🔍 APS Calculator: Analyzing ${ALL_SOUTH_AFRICAN_UNIVERSITIES.length} universities for APS ${totalAPS}`);
    }

    // Always show programs, even if user hasn't entered APS yet
    ALL_SOUTH_AFRICAN_UNIVERSITIES.forEach(university => {
      const universityProgramCount = university.faculties?.reduce((total, fac) => total + (fac.degrees?.length || 0), 0) || 0;
  ...
  const universityMatches = useMemo(() => {
    ...
    ALL_SOUTH_AFRICAN_UNIVERSITIES.forEach(university => {
      const totalPrograms = university.faculties?.reduce((total, faculty) =>
        total + (faculty.degrees?.length || 0), 0) || 0;

      const eligiblePrograms = university.faculties?.reduce((total, faculty) =>
        total + (faculty.degrees?.filter(degree => totalAPS >= degree.apsRequirement).length || 0), 0) || 0;

      const allAPS = university.faculties?.flatMap(faculty =>
        faculty.degrees?.map(degree => degree.apsRequirement) || []) || [];

      const averageAPS = allAPS.length > 0 ? Math.round(allAPS.reduce((sum, aps) => sum + aps, 0) / allAPS.length) : 0;
  ...
  const stats = useMemo(() => {
    ...
    const eligibleCount = totalAPS > 0 ? degreeAnalysis.filter(d => d.eligible).length : 0;
    const eligibilityRate = totalDegrees > 0 && totalAPS > 0 ? Math.round((eligibleCount / totalDegrees) * 100) : 0;

    const topUniversities = universityMatches.filter(u => u.eligiblePrograms > 0).length;
    const averageRequirement = totalDegrees > 0
      ? Math.round(degreeAnalysis.reduce((sum, d) => sum + d.apsRequirement, 0) / totalDegrees)
      : 0;

    const performancePercentile = isNaN(totalAPS) || totalAPS === 0
      ? 0
    ...
    return {
      ...
      performancePercentile: isNaN(performancePercentile) ? 0 : performancePercentile
    };
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className="text-center mb-8">
            ...
            <h1 className="text-4xl font-bold mb-4">Professional APS Calculator</h1>
            <p className="text-xl text-slate-300 max-w-3xl mx-auto leading-relaxed">
              Calculate your Admission Point Score (APS) and discover which university programs you qualify for.
              Enter your matric marks to see your eligibility across all South African universities.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            <div className="bg-slate-800 rounded-xl p-6 text-center border border-slate-700">
              ...
              <div className="text-3xl font-bold text-white mb-1">{totalAPS}</div>
              <div className="text-slate-300 text-sm uppercase tracking-wider">Current APS</div>
            </div>
            <div className="bg-slate-800 rounded-xl p-6 text-center border border-slate-700">
              <Target className="h-8 w-8 text-blue-400 mx-auto mb-3" />
              <div className="text-3xl font-bold text-white mb-1">{stats.eligibilityRate}%</div>
              <div className="text-slate-300 text-sm uppercase tracking-wider">Eligibility Rate</div>
            </div>
            <div className="bg-slate-800 rounded-xl p-6 text-center border border-slate-700">
              <Building className="h-8 w-8 text-purple-400 mx-auto mb-3" />
              <div className="text-3xl font-bold text-white mb-1">{stats.topUniversities}</div>
              <div className="text-slate-300 text-sm uppercase tracking-wider">Universities</div>
            </div>
            <div className="bg-slate-800 rounded-xl p-6 text-center border border-slate-700">
              <GraduationCap className="h-8 w-8 text-orange-400 mx-auto mb-3" />
              <div className="text-3xl font-bold text-white mb-1">{stats.eligibleCount}</div>
              <div className="text-slate-300 text-sm uppercase tracking-wider">Programs</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
              ...
              <CardHeader className="border-b border-slate-100 pb-4">
                <CardTitle className="flex items-center gap-3 text-slate-900">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-slate-500">Enter your marks below</div>
                  </div>
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-6 space-y-4">
                        ...
                        >
                          ...
                          <SelectContent>
                            {subject.name !== "Select Subject" && (
                              <SelectItem key={`current-${subject.name}`} value={subject.name}>
                                {subject.name}
                              </SelectItem>
                            )}
                            {availableSubjects.map((availableSubject) => (
                              <SelectItem key={availableSubject} value={availableSubject}>
                                {availableSubject}
                              </SelectItem>
                            ))}
                          </SelectContent>
                      ...
                      <div className="flex w-full sm:col-span-5 gap-2 items-center">
                        ...
                        <div className="flex-shrink-0">
                          <div className={cn(
                            ...
                            subject.points >= 5 ? "bg-emerald-100 text-emerald-700" :
                            subject.points >= 3 ? "bg-orange-100 text-orange-700" :
                ...
                <div className="mt-8 p-6 bg-gradient-to-r from-emerald-50 to-blue-50 rounded-xl border border-emerald-200">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-slate-900 mb-2">{totalAPS}</div>
                    <div className="text-slate-600 mb-4">Total APS Score</div>
                    <Badge className={performance.color} variant="secondary">
                      {performance.level}
                    </Badge>
                    <div className="text-sm text-slate-600 mt-2">{performance.description}</div>
                  </div>

                  <div className="mt-6 space-y-3">
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-600">Performance Percentile</span>
                      <span className="font-medium text-slate-900">{stats.performancePercentile}%</span>
                    </div>
                    <Progress value={stats.performancePercentile} className="h-2" />
                  </div>
                </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="p-4 bg-orange-50 rounded-lg border border-orange-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-orange-900">Opportunities</span>
                  </div>
                ...
                <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-emerald-900">Next Steps</span>
                  </div>
                  <div className="text-sm text-emerald-800">
                    Explore {stats.eligibleCount} eligible programs and connect with university advisors
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="lg:col-span-2">
            <Tabs value={activeInsight} onValueChange={(value) => setActiveInsight(value as 'overview' | 'universities' | 'programs')} className="w-full">
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-xl font-bold text-slate-900">{stats.totalDegrees}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-slate-600">
                            {totalAPS === 0 ? 'Enter APS to see eligible' : 'Eligible Programs'}
                          </span>
                          <span className={`text-xl font-bold ${totalAPS === 0 ? 'text-slate-400' : 'text-emerald-600'}`}>
                          ...
                          <div className="pt-2">
                            <div className="flex justify-between text-sm mb-2">
                              ...
                              <span className="font-medium">
                                {totalAPS === 0 ? 'Calculate APS first' : `${stats.eligibilityRate}%`}
                              </span>
                            </div>
                            <Progress value={totalAPS === 0 ? 0 : stats.eligibilityRate} className="h-3" />
                          </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-xl font-bold text-slate-900">{totalAPS}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-slate-600">Average Required</span>
                          <span className="text-xl font-bold text-slate-600">{stats.averageRequirement}</span>
                        </div>
                        <div className="pt-2">
                          <div className="flex justify-between text-sm mb-2">
                          ...
                          <Progress
                            value={isNaN(totalAPS) || totalAPS === 0 ? 0 : Math.min(100, (totalAPS / 42) * 100)}
                            className="h-3"
                          />
                        </div>
                      </div>
                ...
                <Card className="bg-white border-0 shadow-lg">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 gap-4">
                      {(showAllUniversities ? universityMatches : universityMatches.slice(0, 6)).map((university, index) => (
                        <div key={index} className="p-4 border border-slate-200 rounded-lg hover:shadow-md transition-shadow">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <h4 className="font-semibold text-slate-900">{university.abbreviation}</h4>
                              <p className="text-sm text-slate-600">{university.location}</p>
                            </div>
                            <Badge
                              variant="outline"
                              className={cn(
                                university.competitiveness === 'High' ? 'border-red-200 text-red-700 bg-red-50' :
                                university.competitiveness === 'Moderate' ? 'border-orange-200 text-orange-700 bg-orange-50' :
                          ...
                          <div className="text-sm text-slate-600">
                            {university.eligiblePrograms} of {university.totalPrograms} programs available
                          </div>
                            <Progress
                              value={
                                university.totalPrograms === 0
                                  ? 0
                                  : Math.min(100, (university.eligiblePrograms / university.totalPrograms) * 100)
                              }
                              className="h-2 mt-2"
                            />
                        </div>
                    ...
                    {universityMatches.length > 6 && !showAllUniversities && (
                      <div className="text-center mt-6">
                        ...
                        >
                          View More Universities ({universityMatches.length - 6} more)
                        </Button>
                      </div>
                    )}
                ...
                <Card className="bg-white border-0 shadow-lg">
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between text-slate-900">
                      ...
                      <Badge variant="outline">{universityMatches.filter(u => u.eligiblePrograms > 0).length} matches</Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {universityMatches.filter(u => u.eligiblePrograms > 0).slice(0, 8).map((university, index) => (
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <div className="min-w-0 flex-1">
                                  <h4 className="font-semibold text-slate-900 truncate">{university.university}</h4>
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm">
                                <span className="text-slate-600">
                                  <span className="font-medium text-emerald-600">{university.eligiblePrograms}</span> eligible programs
                                </span>
                                <span className="text-slate-600">
                                  Avg APS: <span className="font-medium">{university.averageAPS}</span>
                                </span>
                                <Badge
                                  variant="outline"
                                  className={cn(
                  ...
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between text-slate-900">
                      <div className="flex items-center gap-2">
                        ...
                        {totalAPS === 0 ? 'Available Programs' : 'Program Matches'}
                      </div>
                      <Badge variant="outline">
                        {filteredDegrees.length} {totalAPS === 0 ? 'samples' : 'results'}
                      </Badge>
                    </CardTitle>
                    {totalAPS === 0 && (
                      <CardDescription className="text-slate-600">
                        Enter your marks above to see which programs you're eligible for. Below is a sample of available programs.
                      </CardDescription>
                    )}
                  </CardHeader>
                  <CardContent>
                    {filteredDegrees.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <h3 className="text-lg font-semibold text-slate-600 mb-2">No Programs Available</h3>
                        ...
                        <div key={index} className="p-4 border border-slate-200 rounded-lg hover:shadow-md transition-shadow">
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
                            <div className="flex-1 min-w-0">
                              <h4 className="font-semibold text-slate-900 mb-1">{degree.name}</h4>
                              <p className="text-sm text-slate-600 mb-2">{degree.university} • {degree.faculty}</p>
                              <p className="text-sm text-slate-700">{degree.description}</p>
                            </div>
                            <div className="flex flex-row sm:flex-col items-center sm:items-end gap-2">
                              <Badge
                                variant={degree.eligible ? 'default' : 'destructive'}
                              ...
                              <Badge
                                ...
                                className={cn(
                                  "whitespace-nowrap",
                                  degree.competitiveness === 'High' ? 'border-red-200 text-red-700 bg-red-50' :
                                  degree.competitiveness === 'Moderate' ? 'border-orange-200 text-orange-700 bg-orange-50' :
                                  'border-green-200 text-green-700 bg-green-50'
                                )}
                              >
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-white border-0 shadow-lg hover:shadow-xl transition-shadow cursor-pointer"
            ...
            <CardContent className="p-6 text-center">
              ...
              <h3 className="font-semibold text-slate-900 mb-2">Find Bursaries</h3>
              <p className="text-sm text-slate-600">Discover funding opportunities for your studies</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-0 shadow-lg hover:shadow-xl transition-shadow cursor-pointer"
            ...
            <CardContent className="p-6 text-center">
              ...
              <h3 className="font-semibold text-slate-900 mb-2">Browse Textbooks</h3>
              <p className="text-sm text-slate-600">Find books for your chosen programs</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-0 shadow-lg hover:shadow-xl transition-shadow cursor-pointer"
            ...
            <CardContent className="p-6 text-center">
              ...
              <h3 className="font-semibold text-slate-900 mb-2">University Guide</h3>
              <p className="text-sm text-slate-600">Explore universities and programs in detail</p>
            </CardContent>
          </Card>
        </div>
      </div>
